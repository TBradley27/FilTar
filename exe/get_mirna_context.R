...
mirna_seeds = read_table2(commandline_args[2], col_names=c("identifier", "seq"))
...
specific_tax_id = TaxID[[commandline_args[1]]]
...
mirnas = read_tsv(commandline_args[3], col_names = c('miRNA_family','tax_id','miRNA','miRNA_sequence'))
...
x = mirnas[!mirnas$miRNA_family %in% test$miRNA_family,]
print(test)
print(x)
write_tsv(test, commandline_args[4], col_names=FALSE)
