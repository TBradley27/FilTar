...
run_test = function(input, species ) {
	...
	write.table(output, str_interp('${species}_mock3.tsv'), col.names=TRUE,row.names=FALSE, quote=FALSE, sep="\t")	
	...
	if (species == 'hsa') {
		...
		test_that('The output does not contain seed sequences without a reference orthologue', {
                        expect_equal(dim(output %>% filter(seq=='AGCGCGC'))[1], 0)
                })
	}
}

run_test('mock.tsv','hsa')
run_test('mock.tsv', 'mmu')
