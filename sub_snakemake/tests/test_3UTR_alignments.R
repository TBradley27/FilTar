...
all_tests = function (species, taxonomic_id) {
	...
	canon_tax_ids = read_tsv('../../config/species_basic.tsv', col_names=c('species','tax_id'))
	bed_file = read_tsv( str_interp('../../results/${species}_3UTR_biopython.chrY.bed'), col_names=FALSE)

	tax_ids = alignment$tax_id %>% unique()

	print(tax_ids)

	#alignment$tx_id %in% bed_file$X4 %>% print()
	...
	test_that("The tax_id column only contains recognised taxonomic IDs", {
	...
	biomart_data = read_tsv(str_interp('${species}_mart_export.tsv'), col_names=c('tx_id','seq'))
