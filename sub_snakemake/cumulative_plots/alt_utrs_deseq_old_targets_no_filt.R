...
nontargets$legend = stringr::str_interp("No seed site (n=${length(non_targets_exp2)})")
...
ggplot_object = ggplot(
  ...
  scale_color_manual(
                 values=c("black","firebrick1","darkorange"),
                 breaks=c(    # change legend order
                        ...
                        stringr::str_interp("No seed site (n=${length(non_targets_exp2)})")
                        )
		) +
