...
# read in the data
normal_bed = read_tsv(snakemake@input$normal_bed, col_names=c('chromosome','start','stop','strand','id'), col_types=list('c','i','i','c','c'))
extended_utrs = read_tsv(snakemake@input$extended_bed, col_names=c('chromosome','start','stop','id','dummy','strand'))
...
# write
write.table(reannotated_utrs_sorted, file=snakemake@output[[1]], quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")
